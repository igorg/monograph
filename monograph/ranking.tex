...
Para a \emph{função \ref{func:combinar}}, o melhor caso ocorre quando $k = 1$ ou $k = n - 1$ com tempo computacional de $O(1)$. Nesses casos, todas as instâncias pertencem à mesma classe, exceto por uma e $S_{\alpha} = \emptyset$ ou $S_{\beta} = \emptyset$, dessa forma os loops não são executados.

Na analise dos casos extremos, em que $k = 0$ ou $k = n$, todas as instâncias pertencem a mesma classe. Nesses casos, o algoritmo não conseguiria formar pares para treinar um classificador, logo esses casos não estarão incluídos em nossos estudos.
...
\begin{align*}
    f_{avg}(n) &= \frac{1}{n - 1} \cdot \sum_{k = 1}^{n - 1} k(n - k) \\
               ...
               &= \frac{1}{n - 1} \cdot \left(n \cdot \frac{n(n + 1)}{2} - \frac{n(n + 1)(2n + 1)}{6}\right) \\
               &= \frac{1}{n - 1} \cdot \frac{n^3 + n}{6} = \frac{n^2 -1}{6}
\end{align*}
...
Amostragem consiste em utilizar apenas um subconjunto da base completa a ser usada no treinamento do classificador. O objetivo da estratégia não é reduzir a complexidade intríseca das etapas de particionamento e combinação, mas limitar a quantidade de instâncias a serem combinadas amortizando o que seria o tempo total de geração das novas instâncias.
...
Há uma complementaridade entre as duas estratégias escolhidas para otimizar o tempo do \emph{algoritmo \ref{alg:auc-train}}. Enquanto uma preza por melhor tempo de execução, o outra preza por maior segurança na classificação. Basta combiná-las para balancear compromisso entre tempo de execução e menor erro na classificação.
...
Não faz sentido uma votação entre classificadores treinados a partir da base completa.


O algoritmo funciona para o algoritmo original do \emph{AUC-Train} e para as otimizações também.
...
\section{Avaliação: \emph{Tournament} e Otimizações}
