...
As bases usadas foram as seguintes: Breast Cancer; Statlog (Vehicle Silhouettes), chamada de Vehicle; Hepatitis; Glass Identification, chamada de Glass e Yeast. Como o algoritmo proposto em \cite{langford08} tem um custo computacional alto, $O(n^2)$ para treinamento e $O(n^2)$ para gerar o ranking; isso aliado ao baixo controle sobre o ambiente de execução culminou na escolha de bases pequenas para avaliação do experimento.
...
\begin{enumerate}
    ...
    \item O classificador como base para a técnica de \emph{ranking reduzido a classificação} original (ranking);
    \item O classificador como base para o algoritmo de ranking com configurações de 1 par por instância e variando o número de classificadores na votação entre 1 e 20 (número de classificadores);
    \item O classificador como base para o algoritmo de ranking com configurações de 1 classificador na votação e variando o número de pares por instância entre 1 e 20 (número de pares).
\end{enumerate}

Nas três baterias que envolveram o algoritmo de \emph{ranking}, o método de ordenação usado para gerar o resultado final foi o \emph{torneio}, como explicado no artigo \cite{langford08}. Em todas as baterias foi executada uma validação cruzada com 10 partições e os resultados apresentados nas tabelas é a média das \emph{AUCs} obtidas para cada partição.
...
\begin{figure}[h!]
    ...
    \caption{Gráficos de desempenho para árvore de decisão C4.5\\ Eixo y: AUC --- eixo x: número de pares ou classificadores}
\end{figure}
...
\begin{figure}[h!]
    ...
    \caption{Gráficos de desempenho para Naïve Bayes\\ Eixo y: AUC --- eixo x: número de pares/ número de classificadores}
\end{figure}
...
\begin{figure}[h!]
    ...
    \caption{Gráficos de desempenho para Curva Logística \\ Eixo y: AUC --- eixo x: número de pares/ número de classificadores}
\end{figure}
...
\begin{figure}[h!]
    ...
    \caption{Gráficos de desempenho para Support Vector Machine \\ Eixo y: AUC --- eixo x: número de pares/ número de classificadores}
\end{figure}